...
- months: מערך "YYYY-MM" של כל החודשים הרלוונטיים. עונות: חורף=דצמבר+ינואר+פברואר, אביב=מרץ+אפריל+מאי, קיץ=יוני+יולי+אוגוסט, סתיו=ספטמבר+אוקטובר+נובמבר. חלקי עונה: "תחילת/ראשית X"=החודש הראשון של העונה בלבד, "אמצע X"=החודש האמצעי, "סוף/שלהי X"=החודש האחרון (למשל "סוף הקיץ"→אוגוסט בלבד, "תחילת החורף"→דצמבר). אם לא צוין זמן — []
...
function translateLocal(text){
  ...
  const _seasonPart=(name,arr)=>{ // "סוף הקיץ" → אוגוסט בלבד
    ...
    if(pre("סוף")||pre("שלהי")) return [arr[2]];
    if(pre("אמצע")) return [arr[1]];
    if(pre("תחילת")||pre("ראשית")||pre("בתחילת")) return [arr[0]];
    return arr;
  };
  for(const [he,list] of Object.entries(SEASONS)){ if(has(he)){ _seasonPart(he,list).forEach(mm=>I.months.push(_futureYM(mm))); } }
